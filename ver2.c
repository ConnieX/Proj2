...
int validate_x(const char* argv, double** d){
    //if (strstr(argv, "INF"))
    //    return 1;
...
//function: validate_number - control of right input
int validate_y(const char* argv, double** d){
    //if (strstr(argv, "INF"))
    //    return 1;
    int size = strlen(argv); //lenght of argv
    int decpoint = 0;
...
int try_input_to_num_double(char* argv, double* d){
    //printf("%s", argv);
    double i;
    if(!validate_x(argv, &d) && !validate_y(argv, &d) || !sscanf(argv, "%lg", &i)){
        fprintf(stderr, "Wrong argument: %s \nNuber must be greater or equal to 0.\nPlease use decimal point.\n", argv);
        return 0;
    }
    if ((i > LONG_MAX) && i != INFINITY){
...
int main(int argc, char* argv[]){
    if(argc == 4){
        ...
        double x;
        if(!try_input_to_num_double(argv[2], &x))
            return 0;
        unsigned int n;
        ...
        if(!try_input_to_num_double(argv[2], &x))
            return 0;
        double y;
        if(!try_input_to_num_double(argv[3], &y))
            return 0;
        unsigned int n;
